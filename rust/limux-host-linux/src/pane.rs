...
fn add_terminal_tab_inner(
    ...
    // Prefer the per-tab Claude resume command (proc-scan based, hook-free);
    // fall back to upstream's agent resume command when present.
    ...
    if let Some(command) = startup_command.as_deref() {
        eprintln!(
            "limux: restoring terminal surface={}:{} command={}",
